        ...
        <tbody class="divide-y divide-slate-800/60">
          <tr v-for="wallet in financeStore.wallets" :key="wallet.walletId" class="hover:bg-slate-800/40 transition-colors">
            ...
            <td class="px-6 py-4 text-sm font-medium text-teal-400">
              {{ new Intl.NumberFormat('lo-LA').format(Number(wallet.balance)) }}
            </td>
            <td class="px-6 py-4">
              <span
...
const openTopUpModal = (wallet: Wallet) => {
...
const handleTopUpSubmit = async () => {
  ...
  try {
    ...
    topUpModal.error = getErrorMessage(err, 'Failed to top up wallet');
  } finally {
